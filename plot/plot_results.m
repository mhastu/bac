...
for dev=Devs
    ...
    switch(method)
        case 'rep'
            calib_significance = adjust_chance_level(1/C, calib_cut*num_trials_per_sys(dev)/np,alpha,T);
        case 'CP'
            calib_significance = adjust_chance_level(1/C, calib_cut*num_trials_per_sys(dev),alpha,T);
        case 'LS'
            calib_significance = adjust_chance_level(1/C, sum(num_trials_per_sys([1:dev-1 dev+1:3])),alpha,T);
        otherwise
    ...
    if ~strcmp(method, 'LS')  % no STD for leaveout-system
        ...
        calib_legend_entries = {'mean', 'cue', ['significance: ' num2str(calib_significance*100, '%.1f') '%'], ['max acc: ' num2str(calib_pmean_peak_accuracy*100, '%.1f') '%']};
    end
    ...
    switch(method)
        case 'rep'
            test_significance = adjust_chance_level(1/C, (1-calib_cut)*num_trials_per_sys(dev)/np,alpha,T);
        case 'CP'
